...
#Top20_24 has been obtained in annotatingres24.sh#
HTMAP=assay(rld)[Top20_24$ensembl_gene_id,]
colnames(HTMAP)=row.names(sample_table)
row.names(HTMAP)=Top20_24$external_gene_name
...
pheatmap(HTMAP, color=myDivergePal, fontsize_row = 6,number_color = "black" ,annotation_col = heatmap_annotation,display_numbers = TRUE, annotation_colors = mat_colors, scale="row",annotation_legend = TRUE, main = "Regular Log Transformed Fold Change for the top 20 genes - Within Gene Z scores")
